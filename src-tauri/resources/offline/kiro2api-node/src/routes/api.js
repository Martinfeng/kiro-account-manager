...
function isSonnet46Model(value) {
  const text = String(value || '').toLowerCase();
  if (!text) return false;
  return (
    text.includes('claude-sonnet-4.6') ||
    /sonnet[-_.]?4[-_.]?6(?:\b|[-_.]\d)/i.test(text)
  );
}

// 获取模型上下文长度（自适应）
function getModelContextLength(model, upstreamModel, config) {
  ...
  // 优先使用映射后的上游模型 ID 判断
  if (isSonnet46Model(upstreamModel) || isSonnet46Model(model)) {
    return 1000000;
  }

  const lower = String(upstreamModel || model || '').toLowerCase();
...
async function handleStreamResponse(res, response, toolNameMap, selected, state, startTime, model, req, upstreamModel) {
  ...
  let inputTokens = 0;
  const modelContextLength = getModelContextLength(model, upstreamModel, state.config);
  let outputTokens = 0;
