...
export function createAdminRouter(state) {
  ...
  const rejectWhenSharedMode = (req, res, next) => {
    if (state.accountPool.isSharedMode && state.accountPool.isSharedMode()) {
      return res.status(409).json({
        error: '共享账号池模式下账号列表为只读，请在 Kiro Account Manager 中管理账号'
      });
    }
    next();
  };
  ...
  router.get('/status', (req, res) => {
    ...
    res.json({
      ...
      sharedAccountsFile: state.accountPool.getSharedAccountsFile
        ? state.accountPool.getSharedAccountsFile()
        : null
    });
  });
  ...
  // POST /api/accounts
  router.post('/accounts', rejectWhenSharedMode, async (req, res) => {
    try {
  ...
  router.post('/accounts/import', rejectWhenSharedMode, async (req, res) => {
    try {
  ...
  router.delete('/accounts/:id', rejectWhenSharedMode, async (req, res) => {
    const removed = await state.accountPool.removeAccount(req.params.id);
    res.status(removed ? 204 : 404).end();
  });

  // POST /api/accounts/:id/enable
  router.post('/accounts/:id/enable', rejectWhenSharedMode, async (req, res) => {
    const success = await state.accountPool.enableAccount(req.params.id);
    res.json({ success });
  });

  // POST /api/accounts/:id/disable
  router.post('/accounts/:id/disable', rejectWhenSharedMode, async (req, res) => {
  ...
  router.delete('/accounts/batch', rejectWhenSharedMode, async (req, res) => {
    const { ids } = req.body;
