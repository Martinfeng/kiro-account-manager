...
export class TokenManager {
  constructor(config, credentials) {
    ...
    this.expiresAt = expiresAt ? new Date(expiresAt) : new Date(0);
  }
  ...
  async refreshSocialToken() {
    ...
    const fetchOptions = {
      ...
      // 同时兼容 refreshToken / refresh_token 两种字段风格
      body: JSON.stringify({
        refreshToken,
    ...
    if (data.refreshToken || data.refresh_token) {
      ...
      this.credentials.refresh_token = this.credentials.refreshToken;
    }
